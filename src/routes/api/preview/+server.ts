...
import {
	type ProjectNoContent,
	projectNoContentQuery,
...
export const GET = (async ({ url: { searchParams }, cookies, setHeaders }) => {
	...
	switch (type) {
		case "project":
			document = await getClient(true).fetch<ProjectNoContent>(
				projectNoContentQuery,
				{ slug },
			);
			...
			redirectSlug = `/projects/${document.slug}${
				isEmbed ? "?isEmbedPreview=true" : ""
			}`;
			break;
